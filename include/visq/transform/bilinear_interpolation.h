...
public:
  ...
  [[nodiscard]] T Interpolate(double y, double x, size_t channel) const override {
    ...
    double value_at_top = InterpolateOnZeroOneInterval(this->image_.At(ty, rx, channel), this->image_.At(ty, lx, channel), x - rx);
    double value_at_bottom = InterpolateOnZeroOneInterval(this->image_.At(by, rx, channel), this->image_.At(by, lx, channel), x - rx);

    double result = InterpolateOnZeroOneInterval(value_at_top, value_at_bottom, y - ty);

    result = std::max(static_cast<double>(std::numeric_limits<T>::min()), std::min(static_cast<double>(std::numeric_limits<T>::max()), result));
